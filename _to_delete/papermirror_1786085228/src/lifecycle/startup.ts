...
export async function startup(params: StartupParams): Promise<void> {
	...
	const publicAPI = {
		...
		diagnose: async (): Promise<string> => {
			...
			lines.push(`Provider: ${getPref<string>('provider', 'bing-free')}, model: ${getPref<string>('model', '') || '(default)'}`);
			lines.push(`API key configured: ${(await getApiKey(getPref<string>('provider', 'bing-free'))).length > 0}`);
