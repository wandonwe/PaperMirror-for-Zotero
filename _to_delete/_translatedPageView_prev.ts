...
function localPaper(ctx: CanvasRenderingContext2D, box: { left: number; top: number; width: number; height: number }, scaleFactor: number, fallback: string): string {
	try {
		const x1 = Math.round((box.left + 1.5) * scaleFactor);
		const y1 = Math.round((box.top + 1.5) * scaleFactor);
		const x2 = Math.round((box.left + box.width - 1.5) * scaleFactor);
		const y2 = Math.round((box.top + box.height - 1.5) * scaleFactor);
		const xm = Math.round((x1 + x2) / 2);
		const points: [number, number][] = [[x1, y1], [x2, y1], [x1, y2], [x2, y2], [xm, y1], [xm, y2]];
		const counts = new Map<string, number>();
		for (const [x, y] of points) {
		...
		return best && bestCount >= 2 ? `rgb(${best})` : fallback;
	}
	catch {
		return fallback;
	}
...
export function buildTranslatedPage(
	...
	// Match the ORIGINAL's on-screen size, never exceed it: shrink to fit the
	// pane if needed, but never grow beyond 1:1 with the reader's render —
	// 两页大小一致 wins.
	const pxPerViewport = Math.min(input.availableWidth / viewportWidth, 1);
