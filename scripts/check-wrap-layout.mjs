...
try {
 ...
 writeFileSync(join(temporary,'check.html'), '<!doctype html><meta charset="utf-8"><style>'+readFileSync(join(root,'src/ui/styles/translationPane.css'),'utf8')+'</style><script>window.escPanelImage="data:image/png;base64,'+readFileSync(join(root,'tests/fixtures/regression/esc2024-p8-panels.png')).toString('base64')+'";window.captionFixtureImage="data:image/png;base64,'+readFileSync(join(root,'tests/fixtures/regression/simohamed2021-p4.png')).toString('base64')+'";window.realTranslationImages='+JSON.stringify(Object.fromEntries([35,40,27,22,38,19].map(p=>[p,'data:image/png;base64,'+readFileSync(join(root,`tests/fixtures/regression/${p===19?'bae2010':p===22?'serruys2021':'stroke2026'}-p${p}-visual.png`)).toString('base64')])) )+';</script><script defer src="check.js"></script>');
