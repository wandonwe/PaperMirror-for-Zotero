...
test('不把短线延长成边界:只盖住一小段的线不算列边界 (2.12.4)', () => {
	...
	// (2.12.10 起网格至少 2 列 2 行,所以基础网格是 3 竖 3 横。)
	const good: Segment[] = [
		[100, 100, 100, 400], [150, 100, 150, 400], [300, 100, 300, 400],
		[100, 100, 300, 100], [100, 250, 300, 250], [100, 400, 300, 400]
	];
	...
	assert.equal(grid.columns.length, 3, '短竖线不得成为列边界');
});

test('不做笛卡尔积:缺失的分隔线不会被补出来 (2.12.4)', () => {
	// 四条竖线、三条横线 → 3 列 2 行。不得因为"有四个 x 值"就凑出更多行。
	const segs: Segment[] = [
		[100, 100, 100, 400], [160, 100, 160, 400], [230, 100, 230, 400], [300, 100, 300, 400],
		[100, 100, 300, 100], [100, 250, 300, 250], [100, 400, 300, 400]
	];
	const grid = borderGrid(segs, { pageHeight: 500 })!;
	assert.equal(grid.columns.length - 1, 3, '4 条竖线 = 3 列');
	assert.equal(grid.rows.length - 1, 2, '3 条横线 = 2 行,不得补出更多');
});
...
test('共线片段要合并:按行断开的竖线仍是一条列边界 (2.12.4)', () => {
	...
	for (let i = 0; i < 6; i++) {
		...
		segs.push([200, 100 + i * 50, 200, 150 + i * 50]);
		segs.push([300, 100 + i * 50, 300, 150 + i * 50]);
	}
	segs.push([100, 100, 300, 100], [100, 250, 300, 250], [100, 400, 300, 400]);
	const grid = borderGrid(segs, { pageHeight: 500 });
	assert.ok(grid, '断开的共线片段合并后应构成网格');
	assert.equal(grid!.columns.length - 1, 2, '三条竖线 = 2 列');
});
...
test('共线但隔得很远的两截短线不算一条边界 (2.12.4)', () => {
	...
	const grid = borderGrid(segs, { pageHeight: 500 })!;
	assert.equal(grid.columns.length, 3, '两截远隔的短线不得拼成第四条列边界');
});
...
test('斜线不得混进水平线里(有合法竖线时尤其危险) (2.12.4)', () => {
	...
	const grid = borderGrid(segs, { pageHeight: 500 })!;
	assert.equal(grid.rows.length - 1, 2, '斜线不得变成行边界');
});
