...
- 修复大字号表格文字被误当作折行标题、跨列合并的问题。合并须满足纵向叠放与横向重叠，或同一行紧邻片段的条件；类别、定义和推荐措辞保持独立。
- 粗外框与同排分列文字冲突时，保留原始文本区域，避免将多列彩色面板压成一个单元格。
- 根据表题、重复标签与描述间距识别单列说明型表格；根据缩写和释义的对齐关系识别无表头续表。
- 表格遮罩从文字行内部采样背景色，减少彩色格中的白色空洞。

### 译文放置与校验

- 字形越界检查细分到边界字符，减少混合字体导致整段译文被误拒；绕图区域允许有限的内部留白调整，并重新验证完整文本，仍不扩大图片或单元格边界。
- 校验参考文献译文中保留的拉丁作者和编号，拒绝明显串条结果；本地缓存与接口结果使用同一检查。
- 诊断增加字形越界量及排版测量状态，便于区分容量不足和字形边界失败。

### 通用性与验证

- 规则不依赖论文名称或固定页码。新增更换文字、页码、平移和缩放后的回归，覆盖说明型表格及同排大字号文本。
- 1449 项单元及集成测试通过，包含 41 页布局快照和全文字符完整性检查；真实 PDF 底图上的彩色表格双缩放回放通过。
- 保留既有视觉回归限制：部分密集页面在严格边界下仍无法放置全部译文，例如 Stroke 第 35 页 27/31。完整视觉套件未全绿，不通过降低阈值、跨格扩展或截断文本掩盖；未放置内容可通过完整译文入口查看。
- 浏览器回放不能替代 Zotero 真机验证，不承诺任意表格或扫描图片文字均能自动识别。

## [4.1.0] — 2026-09-18

### 正文、公式与完成状态

- 在文本片段合并前分离带有旧字体控制符的独立公式，避免括号、分母等碎片混入上下文翻译；公式边界贯穿段落合并、表格识别和排版。
- 原位排版优先尝试把完整译文放入当前可用区域，再按需要拆分，减少不必要的测量和碎片化。
- 任务结束或滚回页面后保留未放置状态，不再把仍有未显示内容的页面标为全部完成；“查看译文”定位对应页面。

### 显示与 PDF 导出

- 文字适配增加实际字形边界检查，保护中文笔画和拉丁字母下伸部，避免只凭行框高度放行被裁切的文字。
- PDF 导出按字形高度计算基线，先绘制全部遮罩再绘制译文，避免后绘制的遮罩擦掉相邻译文；检测与其他原文区域的相交。
- 对特定旧 PDF 字体中有编码证据、且原文可验证的统计符号进行有限修正；未解析的控制符或占位符不再覆盖正常原文。
- 对被错误合并成表格单元格的复合森林图保留原图，避免破坏研究行、数字、图形和背景之间的对应关系。

### 验证与已知限制

- 类型检查、1436 项单元及集成测试通过，插件构建通过；第三页真实译文通过 Chrome 四档缩放回归。
- 完整视觉回归的 Stroke 第 35 页仍未达到原有放置数量阈值：本轮修改前后均为 27/31。严格字形边界检查下，部分内容可能保留原文；可通过完整译文入口阅读。
- Chrome 回归不能替代 Zotero 真机验证；第三页原始漏显是否彻底消失仍需真机复核。本版不承诺所有公式、表格和扫描文字均可自动识别。
...
- 图注在段落合并前依据标签与连续行建立来源区域，避免被正文栏拆开或吞并；增加 Bae Figure 3 的真实语料回归。
- 正文栏、表格单元格与页眉页脚携带统一的 `SourceRegion`。单元格直接使用表格模型的 PDF 边界，归属本身不新增不翻译规则。
- 正文只在原栏宽和区域内利用下方空白；图注与单元格保持固定边界，图片避让、遮罩内缩与最终几何审计继续生效。
- IR 检查区域角色、边界有效性、同一区域的栏位与边界一致性，以及单元格与表格模型的一致性。
- 更新结构缓存版本，重新提取时生成新的归属信息；不主动清除整库译文缓存。

### 框架与验证

- 更新架构文档，区分结构识别、翻译调度、显示恢复、排版和 PDF 导出，明确各阶段责任与已知限制。
- 类型检查、1401 项测试通过；41 页语料通过已审阅的分段快照与全文字符清单检查；图注归属修复对应的两页分段/栏位基线已更新，无源文字符丢失。
- 保存的真实译文在三个缩放比例下通过浏览器回归，达到既有放置率门槛，最终几何审计未发现重叠。

### 已知限制

- 上游已经混合成一个块的多栏文字，不能只靠后续区域约束自动拆回；本版不是通用视觉版面识别器。
- 放不下的译文仍可能保留原文显示，并通过完整译文入口查看；没有以截断或跨格扩展换取成功率。
- 自动化回归不等同于 Zotero 真机验收；长文滚动恢复、复杂表格和覆盖模式仍需安装后确认。PDF 导出使用独立管线，不保证与对照视图逐像素一致。

## [3.3.0] — 2026-09-16

本版在 3.2.1 之上合入 GPT6 的 17 次修改(e1bd74d … 1f5fdb8),发布前在干净副本重放 CI 链:
类型检查、1396 项测试全绿(3.2.1 为 1335 项)。

### 表格

- **边框表识别**:独立的边框连通分量分别成表(推荐表、并排小表不再合成一张或只取一张);
  从缺内线的推断表框中恢复正确边界;竖向页面家具(裁切线、边栏线)不再进缩略语表的格。
- **表格归属**:统一表格归属元数据(`tableOwnership`),同一原文块不再被重复分配到多个表/段落;
  表格列在渲染期保留,不被相邻段落吞并。
- **成对缩略语表**:「缩写 — 全称」两列的词汇表在段落合并前先识别为表,不再被并成一段长文。

### 排版

- **绕图与安全区**:换行后的译文按安全区分段流放,只占原文实际占据的行带,不再占用外接矩形里的空角;
  说明文字压图前用位图核对纸面是否空白,提取元数据本身不再授权覆盖图片。
- **禁止横向扩边**:原文的水平范围是硬约束,段旁空白可能是栏间距,不再当作扩展空间;
  窄栏原文与相邻宽栏分开成块,通栏说明与页脚与正文栏分离。
- **可读性下限**按 PDF 点计算后随页面整体缩放,缩小页面时屏幕像素同步缩小。
- **相邻流区域不重叠**,几何配对对所有对应块复查;嵌入图内的图注可恢复并单独排版。

### 翻译与内容

- **人名/机构名改为翻译**(提示词 v3,替代 2.5.13 起的"专有名词一律原样保留"):作者、机构、厂商、
  产品名按既有通行译名翻译,无通行译名则音译;数值、公式、URL、DOI、型号标识不变。参考文献条目翻译
  标题、作者、期刊名,保留年卷页与 DOI。**提示词版本号提升会使旧缓存全部失效,已翻译文档会重新翻译。**
- 阈值类描述句("≥ 50% 狭窄"之类)不再被启发式公式保护误判为公式而保留原文。

### 阅读体验

- **双向页面位置同步**:PDF 与译文面板互相跟随页内位置(页码 + 页内比例),只响应用户的滚轮/指针/
  键盘意图,不响应程序化滚动,显式操作立即接管方向;栏阅读顺序在同步中保留。
- **恢复已译页面不再触发翻译**:重新打开或切换视图时从会话快照直接恢复,不进调度器、不调用引擎
  (诊断中新增 `displayRestores` 计数)。
- 工具栏按面板宽度自适应;语言芯片显示明确选择的语言(大写语言标签,中文脚本标签紧凑显示)。

### 管线与诊断

- 页面归档(`pageArchive`)和语料 JSONL 增加管线契约字段;文档 IR 增加表格模型;
  拒绝原因改为不含原文/译文文本的稳定代码,可安全进入诊断。
- 新增回归夹具 `tests/fixtures/regression/`(真机译文、栏线、旧右栏、CAD-RADS 流重叠等)。

### 待真机验证

人名翻译策略切换后的作者页效果;双向同步在长文档中的稳定性;绕图段落放回率;多表页面放弃分布;
覆盖翻译模式仍无真机导出。
...
- **绕图覆盖**:译文只放进与图片不相交的最大矩形;几何审计对图片改为绝对判定,不再因"原文矩形本来就压图"
  而放行(3.1.7 让绕图段落进排版后留下的洞:文字盒角上压图的部分会画到图上)。放不下即诚实放弃,
  可在完整文章流里看。
- **表格错位**:单元格带上提取期确定的 PDF 边界,渲染按该边界直接排;单元格**禁止扩边**;格内遮罩向内缩,
  不盖格线;提取期已定格的单元格不再在渲染期二次猜结构。
- **表格识别**:同页多张边框表全部接入(此前只取格数最多的一张);有边线证据的**单向**合并单元格
  (缺内线的相邻格合并,只认一行或一列方向,二维空洞不合并)。
- **漏译**:取消短文字的尺寸过滤(任何非空盒都进排版,放不放得下由量测决定);带链接/邮箱的说明行翻译,
  只有裸标识和期刊卷期定位行保留;参考文献块的回声只在作者串、期刊尾段两种形态下算合格,文献标题碎片照抄
  按没译处理(收回 3.2.0 的放宽)。

### 复核记录

补丁套在干净的 3.2.0 上重放:类型检查、1335 项测试全绿。41 个夹具全文对照 0 丢词,译文侧 +70 词(五个首页
的期刊定位行改为翻译)。ESC 2024 p16、Powers 2019 p4 两张真机边框表改前改后逐格一致。变异验证:加回图片的
原始矩形豁免 → 绕图测试变红;删掉单元格扩边禁令 → 原本无测试察觉,已补结构锁(删掉即变红)。
待真机验证:绕图段落的放回率、多表/合并表头页面的放弃分布、带内框线的提示框是否被误判为表。

## [3.2.0] — 2026-09-14

**3.0 → 3.2 的正式版。** 自 3.0.0 起的改动汇总:译文侧默认全文翻译、只保留能精确认定的标识;覆盖翻译模式与
左右对照共用同一套严格原位替换引擎;文章流视图有了入口与出口;一批"译好了却没放回页面"的缺陷逐个修掉
(小型大写字标题、导出方块符号、短标签与参考文献行的尺寸门槛、单行小字块的行高、绕图排的正文、区域合并丢段落
边界、脚注被排成正文字号);署名行、文献作者串、期刊卷期页码原样返回不再被当成"没翻";并行池只收 LLM 引擎;
Bing 的限流信号终于能被调度器看见;诊断导出记录版式偏好、每个不翻/没显示的块都带原因。

### 本版改动

- **并行预览与真实的池同一口径**:主引擎是 LLM 时,偏好里残留的微软通道仍出现在"预计并行"的芯片里
  (截图:OpenAI 3 · Microsoft 微软翻译 1 · DeepSeek 3 · Gemini 3)。预览现在也走 `buildPool`,免费通道不再出现。
- **参考文献块的回声视为合格**(Mets 2013 p8 真机):文献条目按规则"作者/期刊/年份/DOI 原样保留、标题翻译",
  模型对一条碎片("IEEE Trans Med Imaging 2012;31:"、"6. Mets OM, Buckens CF, Zanen P, et al.")原样返回时,
  它的判断比任何正则都可靠;拒收只换来同一结果的重试和 `unrecovered`(该页 12 条)。正文块的回声规则不变。

### 3.1.10 真机读数(记录)

Mets 2013,池 = OpenAI + DeepSeek + Gemini(微软已被剔除,poolSize 3),按实测速度加权(Gemini 权重 0.25);
8 页 done,142/145 可放回块放回,几何审计 0 违例,无限流、无超时。p8 参考文献页 12 条碎片 `unrecovered`(本版修)。
仍待查:该页参考文献被拆成单行碎片而不是整条(区域合并未合上),两个 7pt 表格格按算放得下却放弃。

## [3.1.10] — 2026-09-14

### 修复:页面基准字号把脚注排大、把正文排小(Mets 2013 p2 / CAD-RADS p1 真机)

页面基准字号的本意是让同一档的正文段落字号一致(带 = 中位数的 0.8–1.25 倍),但此前**所有**段落都被统一到
基准:Mets 2013 p2 一段 2039 字符的 7pt 作者单位脚注被排成 10pt,放不下;反过来 CAD-RADS p1 四条 7pt 脚注
把基准拉到 7,整页 10pt 正文缩成 7pt。现在字号在带外的块(脚注、表注、细则)保留自己的字号,带内的仍统一。
基准本身取"带内最小值"的规则未改(p4 那样 47 段 10pt 正文被三段 8.5pt 拉到 8.5 仍会发生,待定)。

### 3.1.9 真机读数(记录,Bing 单引擎)

Mets 2013,8 页 done:95/104 可放回块放回,几何审计 0 违例;41 次请求全部成功、无限流上报(盲区已修,
这次的 0 是真的 0);同一块的二次请求 4 次。未放回:两段 7pt 脚注(本版修)、"PPV (%)" 这类 25pt 宽的表头
(译文比格宽,正确放弃)、一条字母间带空格的大写标题、两个表格格待查。
