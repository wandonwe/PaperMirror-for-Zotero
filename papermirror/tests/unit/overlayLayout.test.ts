...
test('figures and tables are never covered by the overlay', () => {
	assert.equal(isOverlayableType('paragraph'), true);
	assert.equal(isOverlayableType('heading'), true);
	assert.equal(isOverlayableType('caption'), true);
...
test('expand mode ignores blocks in the other column (no gutter crossing)', async () => {
	...
	// nothing blocks it in its own column -> may reach the page bottom
	assert.equal(availableHeight(box, [box, otherColumn], 800, 'expand'), 700);
});
