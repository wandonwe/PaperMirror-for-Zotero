...
/**
 * FNV-1a(32-bit)over `${id}#${page}`,**末尾补一次雪崩** (2.11.2)。
 *
 * ## 为什么必须补
 *
 * FNV-1a 的最后一步是 `h = (h ^ c) * PRIME`。页号在字符串**末尾**,于是相邻页
 * 只差最后一两个字符 —— 之后再没有任何混淆步骤,分数就只差一个 `PRIME`:
 *
 *   openai#0 → 2550532272    openai#1 → 2567309891    openai#2 → 2584087510
 *                        └── 差 16777619 ──┘      └── 差 16777619 ──┘
 *
 * 服务商之间的分数差是 **10⁹ 量级**,而每页的抖动只有 **10⁷** —— 排名几乎
 * 不动。后果不是"一家通吃"(2.11.1 我这么写过,是错的),而是**成片聚集**:
 * 真机 5 家池的前 24 页归属是
 *
 *   google×4  openai×4  google×2  **gemini×10**  google×4
 *
 * 连续 10 页全落在最慢的那家(gemini 实测 12–16 秒/页,而 google-free 是
 * 0.4 秒)。用户看到的就是"读着读着,连着十几页每页等十几秒" ——
 * 这正是长文档变慢最初被报上来的那个现象。
 *
 * 量化(5 家池,与真机一致):
 *
 *   |          | 30 页分布(降序)   | χ²   | 连续同一家的段数 |
 *   |----------|--------------------|------|------------------|
 *   | 补之前   | 16, 10, 4, 0, 0    | 32.0 | 5                |
 *   | 补之后   | 8, 7, 6, 5, 4      | 1.7  | 24               |
 *
 *   (均匀期望每家 6 页;χ² 自由度 4,5% 临界值 9.49。补之前**两家一页都分不到**。)
 *
 * ## 修法与代价
 *
 * 末尾加一段 fmix32 式的移位-乘法雪崩,让最后一个字节的改动扩散到全部 32 位。
 * 函数仍是纯的、确定的、无依赖的,只是分数分布变好。
 *
 * **代价:一次性重新分流。** 页面缓存的身份键走 `canonicalProviderForPage`,
 * 归属一变,换了服务商的那些页缓存读不回来,要重译一次。这是修这个缺陷
 * 必然要付的,付一次。
 */
function hrwScore(providerId: string, pageIndex: number): number {
	...
	// 雪崩:没有这一段,末位字符的改动只会让结果平移一个 PRIME。
	h ^= h >>> 16;
