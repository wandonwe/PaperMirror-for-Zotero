 ...
 *
 * **2.12.5 更正**: 下面这句"差别不在端点快慢"是**错的**,真机数据已推翻。
 * 2.12.4 的逐请求计时显示,同样大小的载荷(712 vs 716 字符)bing 要 3310 ms、
 * google 只要 166 ms;google 的耗时几乎与内容量无关(5 字符 97 ms,5390 字符
 * 301 ms),bing 则每次往返本身就要约 3 秒。**端点就是慢 20 倍。**
 *
 * 分组本身按设计在工作(2.12.4 真机 p26: 34 个片段 → 9 组,与离线预测的 73%
 * 吻合),但把往返从 34 次降到 9 次,乘上 3 秒的单次延迟仍是 21.8 秒。
 * 所以打包是对的、也该留着,只是它**治不好这一项**:吞吐从 238 涨到 286
 * 字符/秒,跨会话看基本在噪声里。真正起作用的是加权分页 —— bing 的权重已被
 * 实测压到 0.5,它本来就只分到很少的页。
 *
 * 下面这段是 2.12.2 当时的推断,保留以存档,但结论以上面为准:
